...
```{r}
prop.table(table(train$run_pass))
prop.table(table(test$run_pass))
```


```{r}
fit <- rpart( run_pass ~ quarter+time+shotgun+down+yards_to_first+yards_to_endzone, data = train, method = 'class', control = rpart.control("minsplit" = 5))
